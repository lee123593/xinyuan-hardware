export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ---- Products ----
      if (api === '/api/products' && method === 'GET') {
        result = await listProducts(env.DB, url.searchParams);
      } else if (api === '/api/categories' && method === 'GET') {
        result = await listCategories(env.DB);
      } else if (api.startsWith('/api/products/') && method === 'GET') {
        result = await getProduct(env.DB, api.split('/').pop());
      } else if (api === '/api/products' && method === 'POST') {
        result = await createProduct(env.DB, await request.json());
      } else if (api.startsWith('/api/products/') && method === 'PUT') {
        result = await updateProduct(env.DB, api.split('/').pop(), await request.json());
      } else if (api.startsWith('/api/products/') && method === 'DELETE') {
        result = await deleteProduct(env.DB, api.split('/').pop());

      // ---- Orders ----
      } else if (api === '/api/orders' && method === 'GET') {
        result = await listOrders(env.DB, url.searchParams);
      } else if (api === '/api/orders' && method === 'POST') {
        result = await createOrder(env.DB, await request.json());
      } else if (api.startsWith('/api/orders/') && method === 'PATCH') {
        result = await updateOrder(env.DB, api.split('/').pop(), await request.json());

      // ---- Messages ----
      } else if (api === '/api/messages' && method === 'GET') {
        result = await listMessages(env.DB);
      } else if (api === '/api/messages' && method === 'POST') {
        result = await createMessage(env.DB, await request.json());
      } else if (api.startsWith('/api/messages/') && method === 'PATCH') {
        result = await updateMessage(env.DB, api.split('/').pop(), await request.json());

      // ---- Inquiries ----
      } else if (api === '/api/inquiries' && method === 'GET') {
        result = await listInquiries(env.DB);
      } else if (api === '/api/inquiries' && method === 'POST') {
        result = await createInquiry(env.DB, await request.json());
      } else if (api.startsWith('/api/inquiries/') && method === 'PATCH') {
        result = await updateInquiry(env.DB, api.split('/').pop(), await request.json());

      // ---- Stats ----
      } else if (api === '/api/stats' && method === 'GET') {
        result = await getStats(env.DB);

      // ---- Upload ----
      } else if (api === '/api/upload' && method === 'POST') {
        result = await handleUpload(request, env);

      // ---- Health ----
      } else if (api === '/api/health' && method === 'GET') {
...
async function handleUpload(request, env) {
  ...
  const key = 'product_' + Date.now() + Math.random().toString(36).slice(2, 6) + '.' + ext;
